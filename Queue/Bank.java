        ...
        for(time=0;j<n;time++){
            ...
            if(i<n){
                while(clients[i].entertime==time) {
                    if (queue1.length() <= queue2.length()) {
                        ...
                        //System.out.println("c " + clients[i].no + " enters booth" + 1 + " at time " + time);
                        i++;
                    } else if (queue2.length() < queue1.length()) {
                        queue2.enqueue(clients[i]);
                        //System.out.println("c " + clients[i].no + " enters booth" + 2 + " at time " + time);
                        i++;
                    }
                    if(i==n){
                        break;
            ...
            if( delay1==0  ) {
                if (queue1.length() != 0) {
                    ...
                    // System.out.println("c "+ temp.no+" leaves booth"+ 1+" at time "+ queue1time.rearValue());
            ...
            if( delay2==0  ) {
                if (queue2.length() != 0) {
                    ...
                    // System.out.println("c "+ temp.no+" leaves booth"+ 2+" at time "+ queue2time.rearValue());
                    // j++;
                }
            }
            if(delay1!=0){
